...
def td_indicators(symbol):
    # Yahoo format: RELIANCE.NS  TCS.NS  BAJAJ-AUTO.NS  M&M.NS
    # TD format:    NSE:RELIANCE NSE:TCS NSE:BAJAJ-AUTO NSE:MM
    # Use NSE: prefix format — correct for Twelve Data free plan
    ticker = symbol.replace(".NS", "")
    ticker = ticker.replace("&", "")   # M&M → MM,  M&MFIN → MMFIN
    nse_sym = "NSE:" + ticker          # NSE:RELIANCE, NSE:TCS, NSE:MM
...
def phase2_analyze(candidates, market):
    ...
    for c in candidates:
        ...
        if earn["skip"]:
            ...
            continue
        # Twelve Data free plan does not support NSE stocks
        # Yahoo Finance fallback computes identical indicators — use it directly
        ind = None
        if False:  # TD disabled — NSE not available on free plan
            ind = td_indicators(sym)
        if ind is None:
            try:
...
def test_twelvedata():
    ...
    try:
        params = {"symbol": "NSE:RELIANCE", "apikey": TD_KEY}  # correct TD format
