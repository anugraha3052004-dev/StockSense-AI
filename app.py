...
def td_indicators(symbol):
    ...
    # TD free format:  RELIANCE.NSE  TCS.NSE   BAJAJ-AUTO.NSE  MM.NSE
    # Special chars like & and - cause issues in TD — fix them
    ticker = symbol.replace(".NS", "")
    ticker = ticker.replace("&", "")      # M&M → MM,  M&MFIN → MMFIN
    ticker = ticker.replace("-", "-")     # BAJAJ-AUTO stays as-is (TD accepts hyphen)
    ticker = ticker.replace("MCDOWELL-N", "MCDOWELLN")  # special case
...
def test_twelvedata():
    ...
    try:
        params = {"symbol": "RELIANCE.NSE", "apikey": TD_KEY}  # free plan format
        ...
        return jsonify({
            "status": "ok",
            "message": "Twelve Data API is working!",
            "td_key_present": True,
