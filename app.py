...
# ═══════════════════════════════════════════════════════════════════════════════
# ADD-ON MODULE 1 — VIX VELOCITY
# Checks if VIX is cooling (current < 5-day SMA).
# A falling VIX signals fear is reducing — acts as a softener on bad macro score.
# Does NOT override catastrophe (VIX>35 or S&P crash). Only softens -1/-2 scores.
# Zero extra API calls — VIX 5d data fetched here in one shot.
# ═══════════════════════════════════════════════════════════════════════════════

def check_vix_velocity():
    """
    Fetch 5 days of VIX daily closes. Compare current VIX vs its 5-day SMA.
    Returns:
      (is_cooling: bool, current_vix: float, vix_sma5: float, drop_pct: float)
    is_cooling = True means fear is subsiding → use as a macro score softener.
    """
    try:
        df = yf.Ticker("^VIX").history(period="10d", interval="1d")
        close = df["Close"].dropna()
        if len(close) < 5:
            return False, 0.0, 0.0, 0.0
        current_vix = float(close.iloc[-1])
        vix_sma5    = float(close.iloc[-5:].mean())
        is_cooling  = current_vix < vix_sma5
        drop_pct    = round((vix_sma5 - current_vix) / vix_sma5 * 100, 2) if vix_sma5 > 0 else 0.0
        return is_cooling, round(current_vix, 2), round(vix_sma5, 2), drop_pct
    except Exception as e:
        log.debug(f"VIX velocity check failed: {e}")
        return False, 0.0, 0.0, 0.0
...
def run_macro_intelligence():
    ...
    # ── ADD-ON: VIX Velocity check — softens score penalty when fear is cooling ─
    # Does NOT override catastrophe (VIX>35 or S&P crash >3%).
    # Only applies when score is -1 or -2 (mild/moderate bad day).
    vix_cooling, vix_now, vix_sma5, vix_drop = check_vix_velocity()
    if vix_cooling and vix_now < VIX_DANGER and not catastrophe:
        if score in (-1, -2):
            score += 1   # soften by 1 point — fear is receding
            signals.append(
                f"✓ VIX VELOCITY: VIX {vix_now:.1f} < 5-day SMA {vix_sma5:.1f} "
                f"(cooling {vix_drop:.1f}%) · Fear subsiding · Score softened by +1"
            )
            add_log(f"  VIX Velocity: cooling ({vix_now:.1f} < SMA {vix_sma5:.1f}) → score softened to {score}")
        else:
            signals.append(
                f"→ VIX Velocity: cooling ({vix_now:.1f} < SMA {vix_sma5:.1f}) "
                f"but score already {score} — no adjustment needed"
            )
    elif not vix_cooling and vix_now > 0:
        signals.append(
            f"⚠ VIX Velocity: VIX {vix_now:.1f} > 5-day SMA {vix_sma5:.1f} — fear still rising"
        )
    # ─────────────────────────────────────────────────────────────────────────
...
# ═══════════════════════════════════════════════════════════════════════════════
# ADD-ON MODULE 2 — RELATIVE STRENGTH LAGGARD (Catch-up Setup)
# Called inside Phase 2 after base score passes min_score.
# Uses sector_strength data already computed in Stage 3 — zero extra API calls.
#
# Logic: Sector is "strong" (avg 1D% > 1%) BUT this stock is lagging by >1%.
# A lagging stock in a strong sector tends to "catch up" — institutions rotate
# into laggards once leaders are extended. +1.0 bonus for this setup.
# ═══════════════════════════════════════════════════════════════════════════════

def calculate_rs_laggard_bonus(candidate, ss, base_score_passes):
    """
    Check if this stock is a "catch-up laggard" in a strong sector.

    Parameters:
      candidate         : Phase 1 candidate dict (has 'sector', 'chg')
      ss                : sector_strength dict from run_sector_strength_analysis()
      base_score_passes : bool — True only if Phase 2 base score already >= min_score

    Returns:
      (bonus: float, reason: str)
      bonus = +1.0 if laggard catch-up setup confirmed, else 0.0
    """
    try:
        if not base_score_passes:
            return 0.0, ""

        sector    = candidate.get("sector", "OTHER")
        chg_1d    = candidate.get("chg", 0.0)
        strong    = ss.get("strong_sectors", [])
        avg_chg   = ss.get("sector_avg_chg", {}).get(sector, 0.0)

        # Sector must be strong (avg > 1%)
        if sector not in strong or avg_chg <= 1.0:
            return 0.0, ""

        # Stock must be lagging its sector by more than 1%
        lag = avg_chg - chg_1d
        if lag <= 1.0:
            return 0.0, ""

        # Stock must still be positive (not a falling knife)
        if chg_1d < 0:
            return 0.0, ""

        reason = (
            f"🔄 RS LAGGARD CATCH-UP: {sector} sector avg +{avg_chg:.2f}% today "
            f"but stock only +{chg_1d:.2f}% (lagging by {lag:.2f}%) · "
            f"Catch-up rotation likely · +1.0 bonus"
        )
        return 1.0, reason
    except Exception as e:
        log.debug(f"RS laggard check failed: {e}")
        return 0.0, ""


# ═══════════════════════════════════════════════════════════════════════════════
# ADD-ON MODULE 3 — FAIR VALUE GAP (FVG)
# Called in Phase 2 after base score passes, using df already fetched.
# Zero extra API calls — uses the 60-day daily OHLCV already in memory.
#
# Logic: Identify a Bullish FVG in recent candles.
#   A Bullish FVG exists when: C1.High < C3.Low (gap between C1 top and C3 bottom)
#   This is an unfilled institutional imbalance zone.
#   If CMP is within 1% of the TOP of the FVG, price is sitting right at the
#   institutional support — high probability of continuation up.
# Returns +1.5 bonus if condition met.
# ═══════════════════════════════════════════════════════════════════════════════

def calculate_fvg_bonus(df, cmp):
    """
    Scan the last 10 candle triplets for a Bullish FVG.
    A Bullish FVG: C1.High < C3.Low — gap between candle 1 top and candle 3 bottom.
    CMP must be within 1% of the FVG top (C3.Low) — price sitting at support.

    Parameters:
      df  : DataFrame with OHLCV (60d daily, already fetched in Phase 2)
      cmp : Current Market Price (float)

    Returns:
      (bonus: float, reason: str, fvg_top: float)
    """
    try:
        if len(df) < 5:
            return 0.0, "", 0.0
        ...
        # Scan last 12 candles for a bullish FVG
        scan_start   = max(0, len(df) - 12)
        best_fvg_top = 0.0
        best_gap_size = 0.0

        for i in range(scan_start, len(df) - 2):
            c1_high = float(high[i])
            c3_low  = float(low[i + 2])

            # Bullish FVG: gap must exist between C1.High and C3.Low
            if c1_high >= c3_low:
                continue

            gap_size = c3_low - c1_high

            # Gap must be meaningful (at least 0.2% of price)
            if gap_size / cmp < 0.002:
                continue

            # FVG must still be unfilled: C2's close should not have closed below c1_high
            c2_close = float(close[i + 1])
            if c2_close < c1_high:
                continue

            # Track the largest unfilled FVG
            if gap_size > best_gap_size:
                best_gap_size = gap_size
                best_fvg_top  = c3_low   # top of FVG = bottom of C3

        if best_fvg_top == 0.0:
            return 0.0, "", 0.0

        # CMP must be within 1% above the FVG top (sitting right at support)
        proximity_pct = abs(cmp - best_fvg_top) / best_fvg_top * 100
        if proximity_pct > 1.0:
            return 0.0, "", best_fvg_top

        reason = (
            f"🧲 FAIR VALUE GAP: Bullish FVG top at ₹{best_fvg_top:.2f} "
            f"· CMP ₹{cmp:.2f} within {proximity_pct:.2f}% of FVG support "
            f"· Institutional imbalance zone · +1.5 bonus"
        )
        return 1.5, reason, best_fvg_top

    except Exception as e:
        log.debug(f"FVG check failed: {e}")
        return 0.0, "", 0.0
...
#
# BULLISH patterns (9):  Morning Star +2.5 | Bullish Marubozu +2.0 |
#   Rising 3 Methods +2.0 | Hammer +2.0 | Bullish Engulfing +2.0 |
#   Breakout Candle +2.0 | Piercing Line +1.5 | Bullish Harami +1.5 |
#   Gap Up Hold +1.5 | Inside Bar Breakout +1.5 | 3 Green Candles +1.5 |
#   Dragonfly Doji +1.0 | No Upper Wick +1.0
# BEARISH penalties (4): Evening Star -2.5 | Shooting Star -2.0 |
#   Bearish Marubozu -2.0 | Dark Cloud Cover -1.5 | Weak Candle -2.0
# ═══════════════════════════════════════════════════════════════════════════════

# ── Helper: extract last-N candle OHLC values safely ─────────────────────────

def _ohlc(df, i=-1):
    """Return (open, high, low, close, body, range, upper_wick, lower_wick) for candle i."""
    try:
...
# ═══════════════════════════════════════════════════════════════════════════════
# GROUP 1 — REVERSAL PATTERNS  (catch stocks at bottom of a dip)
# ═══════════════════════════════════════════════════════════════════════════════

def morning_star(df):
    """
    3-candle reversal: bearish candle → small doji/spinning top → strong bullish.
    One of the most reliable reversal patterns (~68% win rate on NSE).
    """
    try:
        if len(df) < 3:
            return False
        c1 = _ohlc(df, -3)   # candle 1 — bearish
        c2 = _ohlc(df, -2)   # candle 2 — small/doji
        c3 = _ohlc(df, -1)   # candle 3 — strong bullish
        if not (c1 and c2 and c3):
            return False
        o1,h1,l1,cl1,body1,rng1,uw1,lw1 = c1
        o2,h2,l2,cl2,body2,rng2,uw2,lw2 = c2
        o3,h3,l3,cl3,body3,rng3,uw3,lw3 = c3

        c1_bearish    = cl1 < o1 and body1 / rng1 > 0.5       # strong red candle
        c2_small      = body2 / rng2 < 0.35                    # small body (indecision)
        c2_gaps_down  = max(cl2, o2) < min(cl1, o1)            # gaps below candle 1
        c3_bullish    = cl3 > o3 and body3 / rng3 > 0.5        # strong green candle
        c3_closes_mid = cl3 > (o1 + cl1) / 2                   # closes above midpoint of c1

        return c1_bearish and c2_small and c3_bullish and c3_closes_mid
    except Exception:
        return False
...
def hammer(df):
    """
    Single candle: small body at top, long lower wick (≥2× body), tiny upper wick.
    Strong reversal signal when appearing after a downtrend.
    """
    try:
        if len(df) < 4:
            return False
        o, h, l, c, body, rng, upper_wick, lower_wick = _ohlc(df, -1)
        if rng == 0 or body == 0:
            return False

        # Prior trend: stock must have been falling (any 2 of last 3 closes declining)
        c2 = float(df["Close"].iloc[-2])
        c3 = float(df["Close"].iloc[-3])
        c4 = float(df["Close"].iloc[-4]) if len(df) >= 4 else c3 + 1
        prior_down = (c4 > c3 or c3 > c2)  # at least one declining step

        small_body  = body / rng < 0.35            # small real body
        long_lower  = lower_wick >= 2.0 * body     # lower wick at least 2× body
        tiny_upper  = upper_wick / rng < 0.15      # very small upper wick
        # Hammer valid regardless of colour — key is the long lower wick
        return prior_down and small_body and long_lower and tiny_upper
    except Exception:
        return False
...
def piercing_line(df):
    """
    2-candle reversal: bearish candle → bullish that opens below but closes above midpoint.
    Shows buyers aggressively pushed back sellers.
    """
    try:
        if len(df) < 2:
            return False
        ...
        c1_bearish   = cl1 < o1 and body1 / rng1 > 0.5    # strong red candle
        c2_opens_low = o2 < cl1                             # opens below c1 close
        midpoint_c1  = (o1 + cl1) / 2
        c2_bullish   = cl2 > o2                             # green candle
        c2_closes_mid= cl2 > midpoint_c1 and cl2 < o1      # above mid but below c1 open

        return c1_bearish and c2_opens_low and c2_bullish and c2_closes_mid
    except Exception:
        return False
...
def bullish_harami(df):
    """
    2-candle reversal: large bearish candle → small bullish candle contained inside it.
    Shows selling pressure exhausting — buyers starting to step in.
    """
    try:
        if len(df) < 2:
            return False
        ...
        c1_bearish   = cl1 < o1 and body1 / rng1 > 0.5    # large red candle
        c2_inside    = o2 > cl1 and cl2 < o1               # c2 body inside c1 body
        c2_bullish   = cl2 > o2                             # green
        c2_small     = body2 < body1 * 0.5                 # c2 body < 50% of c1

        return c1_bearish and c2_inside and c2_bullish and c2_small
    except Exception:
        return False
...
def dragonfly_doji(df):
    """
    Open ≈ High ≈ Close, very long lower wick.
    Buyers completely recovered from intraday sell-off — strong support signal.
    """
    try:
        if len(df) < 2:
            return False
        ...
        tiny_body   = body / rng < 0.10             # almost no real body
        tiny_upper  = upper_wick / rng < 0.10       # almost no upper wick
        long_lower  = lower_wick / rng > 0.65       # most of the range is lower wick
        near_support= c > float(df["Close"].iloc[-2])  # closes above prior close

        return tiny_body and tiny_upper and long_lower and near_support
    except Exception:
        return False


# ═══════════════════════════════════════════════════════════════════════════════
# GROUP 2 — CONTINUATION PATTERNS  (momentum will continue)
# ═══════════════════════════════════════════════════════════════════════════════

def bullish_marubozu(df):
    """
    Candle with NO wicks (or tiny wicks) — pure buying from open to close.
    Shows total buyer dominance, one of the strongest single-candle signals.
    """
    try:
        if len(df) < 2:
            return False
        ...
        bullish      = c > o
        large_body   = body / rng > 0.85             # body is 85%+ of range
        tiny_upper   = upper_wick / rng < 0.05       # almost no upper wick
        tiny_lower   = lower_wick / rng < 0.05       # almost no lower wick
        # Require at least 2 candles so prior context exists
        return len(df) >= 2 and bullish and large_body and tiny_upper and tiny_lower
    except Exception:
        return False
...
def rising_three_methods(df):
    """
    5-candle continuation: big green → 3 small reds staying within green → big green.
    Shows consolidation within uptrend — institutions holding while weak hands exit.
    """
    try:
        if len(df) < 5:
            return False
        c1 = _ohlc(df, -5)   # big green
        c2 = _ohlc(df, -4)   # small red 1
        c3 = _ohlc(df, -3)   # small red 2
        c4 = _ohlc(df, -2)   # small red 3
        c5 = _ohlc(df, -1)   # big green
        if not all([c1, c2, c3, c4, c5]):
            return False

        o1,h1,l1,cl1,body1,rng1,uw1,lw1 = c1
        o5,h5,l5,cl5,body5,rng5,uw5,lw5 = c5

        c1_big_green = cl1 > o1 and body1 / rng1 > 0.6
        c5_big_green = cl5 > o5 and body5 / rng5 > 0.6 and cl5 > cl1

        # Middle 3 candles: small and staying within c1's range
        middle_contained = all(
            float(df["High"].iloc[i]) <= h1 and
            float(df["Low"].iloc[i])  >= l1 and
            abs(float(df["Close"].iloc[i]) - float(df["Open"].iloc[i])) < body1 * 0.5
            for i in [-4, -3, -2]
        )
        return c1_big_green and middle_contained and c5_big_green
    except Exception:
        return False


def gap_up_and_hold(df):
    """
    Today's open is higher than yesterday's high (gap up) AND
    price holds above the gap (doesn't fill). Institutional strength.
    """
    try:
        if len(df) < 2:
            return False
        ...
        gap_up      = curr_open > prev_high          # opened above yesterday's high
        gap_held    = curr_low > prev_high * 0.998   # gap not filled (allowing 0.2% tolerance)
        bullish_day = curr_close > curr_open * 0.998 # closed near or above open

        return gap_up and gap_held and bullish_day
    except Exception:
        return False
...
def inside_bar_breakout(df):
    """
    Yesterday's candle was completely inside the day before (low > prior low, high < prior high).
    Today breaks out above the prior high with a bullish candle — compression then explosion.
    """
    try:
        if len(df) < 3:
            return False
        # Candle 3 days ago = mother bar
        mother_high = float(df["High"].iloc[-3])
        mother_low  = float(df["Low"].iloc[-3])
        # Inside bar (2 days ago)
        inside_high = float(df["High"].iloc[-2])
        inside_low  = float(df["Low"].iloc[-2])
        # Today = breakout candle
        curr_close  = float(df["Close"].iloc[-1])
        curr_open   = float(df["Open"].iloc[-1])

        is_inside    = inside_high < mother_high and inside_low > mother_low
        breakout_up  = curr_close > mother_high    # breaks above mother bar
        bullish      = curr_close > curr_open

        return is_inside and breakout_up and bullish
    except Exception:
        return False


# ═══════════════════════════════════════════════════════════════════════════════
# EXISTING PATTERNS (kept exactly as before)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# GROUP 3 — BEARISH PENALTY PATTERNS  (negative score adjustments)
# ═══════════════════════════════════════════════════════════════════════════════

def evening_star(df):
    """
    Opposite of Morning Star: strong green → small doji → strong red.
    One of the most reliable reversal-down signals. Strong penalty.
    """
    try:
        if len(df) < 3:
            return False
        ...
        c1_bullish  = cl1 > o1 and body1 / rng1 > 0.5
        c2_small    = body2 / rng2 < 0.35
        c3_bearish  = cl3 < o3 and body3 / rng3 > 0.5
        c3_deep     = cl3 < (o1 + cl1) / 2

        return c1_bullish and c2_small and c3_bearish and c3_deep
    except Exception:
        return False
...
def shooting_star(df):
    """
    Small body at bottom, very long upper wick. Buyers tried to push higher — rejected hard.
    Appears after uptrend — strong sell signal.
    """
    try:
        if len(df) < 2:
            return False
        ...
        # Prior trend must be up — overall price higher than 2 candles ago
        prior_up   = len(df) >= 2 and float(df["Close"].iloc[-2]) > float(df["Close"].iloc[-3] if len(df)>=3 else 0)
        small_body = body / rng < 0.30
        long_upper = upper_wick >= 2.0 * body
        tiny_lower = lower_wick / rng < 0.15
        # Shooting star valid regardless of candle color (key is rejection)
        return prior_up and small_body and long_upper and tiny_lower
    except Exception:
        return False
...
def bearish_marubozu(df):
    """
    Candle with NO wicks — pure selling. Open is the high, close is the low.
    Total seller dominance — strong penalty.
    """
    try:
        if len(df) < 1:
            return False
        ...
        bearish      = c < o
        large_body   = body / rng > 0.85
        tiny_upper   = upper_wick / rng < 0.05
        tiny_lower   = lower_wick / rng < 0.05

        return bearish and large_body and tiny_upper and tiny_lower
    except Exception:
        return False
...
def dark_cloud_cover(df):
    """
    2-candle: bullish candle → bearish that opens above prior high but closes below midpoint.
    Sellers aggressively overwhelmed buyers at new highs.
    """
    try:
        if len(df) < 2:
            return False
        ...
        c1_bullish    = cl1 > o1 and body1 / rng1 > 0.5
        c2_opens_high = o2 > h1                          # gaps above prior high
        midpoint_c1   = (o1 + cl1) / 2
        c2_bearish    = cl2 < o2 and cl2 <= midpoint_c1  # closes at or below midpoint
        c2_not_full   = cl2 > l1                         # doesn't go below prior low

        return c1_bullish and c2_opens_high and c2_bearish and c2_not_full
    except Exception:
        return False
...
def candle_score(df):
    """
    Score all 13 candlestick patterns on the given OHLC dataframe.
    Returns (total_score, pattern_label_string).

    BULLISH scores:
      Morning Star        +2.5   (reversal — very reliable)
      Bullish Marubozu    +2.0   (pure buying strength)
      Rising 3 Methods    +2.0   (continuation confirmed)
      Hammer              +2.0   (reversal at support)
      Bullish Engulfing   +2.0   (reversal signal)
      Breakout Candle     +2.0   (volume-confirmed breakout)
      Piercing Line       +1.5   (reversal in progress)
      Bullish Harami      +1.5   (sellers exhausting)
      Gap Up Hold         +1.5   (institutional strength)
      Inside Bar Breakout +1.5   (compression → explosion)
      3 Green Candles     +1.5   (sustained momentum)
      Dragonfly Doji      +1.0   (support confirmation)
      No Upper Wick       +1.0   (close near high)

    BEARISH penalties:
      Evening Star        -2.5   (strong reversal down)
      Shooting Star       -2.0   (rejection at high)
      Bearish Marubozu    -2.0   (pure selling)
      Dark Cloud Cover    -1.5   (reversal warning)
      Weak Candle         -2.0   (small bearish body)
    """
    score    = 0.0
    patterns = []
    ...
    # ── BULLISH ───────────────────────────────────────────────────────────────
    if morning_star(df):
        score += 2.5; patterns.append("Morning Star")

    if bullish_marubozu(df):
        score += 2.0; patterns.append("Bullish Marubozu")

    if rising_three_methods(df):
        score += 2.0; patterns.append("Rising 3 Methods")

    if hammer(df):
        score += 2.0; patterns.append("Hammer")

    if bullish_engulfing(df):
        score += 2.0; patterns.append("Bullish Engulfing")

    if strong_breakout_candle(df):
        score += 2.0; patterns.append("Breakout Candle")

    if piercing_line(df):
        score += 1.5; patterns.append("Piercing Line")

    if bullish_harami(df):
        score += 1.5; patterns.append("Bullish Harami")

    if gap_up_and_hold(df):
        score += 1.5; patterns.append("Gap Up & Hold")

    if inside_bar_breakout(df):
        score += 1.5; patterns.append("Inside Bar Breakout")

    if three_green_candles(df):
        score += 1.5; patterns.append("3 Green Candles")

    if dragonfly_doji(df):
        score += 1.0; patterns.append("Dragonfly Doji")

    if upper_wick_rejection(df):
        score += 1.0; patterns.append("No Upper Wick")

    # ── BEARISH PENALTIES ─────────────────────────────────────────────────────
    if evening_star(df):
        score -= 2.5; patterns.append("Evening Star ⚠")

    if shooting_star(df):
        score -= 2.0; patterns.append("Shooting Star ⚠")

    if bearish_marubozu(df):
        score -= 2.0; patterns.append("Bearish Marubozu ⚠")

    if dark_cloud_cover(df):
        score -= 1.5; patterns.append("Dark Cloud Cover ⚠")

    # Weak candle — original check (small bearish body)
    try:
        o = float(df["Open"].iloc[-1]); c = float(df["Close"].iloc[-1])
        h = float(df["High"].iloc[-1]); l = float(df["Low"].iloc[-1])
        rng = h - l
        ...
        if c < o and body_ratio < 0.25:
            score -= 2.0; patterns.append("Weak Candle ⚠")
    except Exception:
        pass
...
def phase2_analyze(candidates, market, scan_mode=SCAN_MODE_INTRADAY, ss=None):
    ...
    for c in candidates:
        ...
        # ── ADD-ON: RS LAGGARD BONUS (Module 2) ──────────────────────────────
        # Check if this stock is a catch-up laggard in its strong sector.
        # base_score_passes is always True here (we only reach this point if score >= min_score)
        rs_bonus, rs_reason = calculate_rs_laggard_bonus(c, ss, base_score_passes=True)
        if rs_bonus > 0:
            reasons.append(rs_reason)
            add_log(f"    RS Laggard bonus +{rs_bonus:.1f}: {sym.replace('.NS','')}")
        # ─────────────────────────────────────────────────────────────────────

        # ── ADD-ON: FAIR VALUE GAP BONUS (Module 3) ───────────────────────────
        fvg_bonus, fvg_reason, fvg_top = calculate_fvg_bonus(df, cmp)
        if fvg_bonus > 0:
            reasons.append(fvg_reason)
            add_log(f"    FVG bonus +{fvg_bonus:.1f}: {sym.replace('.NS','')} FVG top ₹{fvg_top:.2f}")
        # ─────────────────────────────────────────────────────────────────────

        enhanced_score = round(score + c_score + sec_bonus + rs_bonus + fvg_bonus, 2)
        ...
        add_log(
            ...
            f"sec_bonus={sec_bonus:.0f} rs_bonus={rs_bonus:.1f} fvg_bonus={fvg_bonus:.1f} "
            f"enhanced={enhanced_score:.1f} RSI={ind['rsi']:.1f} RR=1:{rr:.1f}"
        )
        results.append({
...
def build_and_send_email(stocks, market, scan_mode=SCAN_MODE_INTRADAY):
    ...
    for s in stocks:
        ...
        plain += (
            ...
            f"(base {s['score']:.1f} + candle {s['candle_score']:.1f} + sector {s['sector_bonus']:.0f}"
            f" + RS lag {s.get('rs_bonus',0):.1f} + FVG {s.get('fvg_bonus',0):.1f})\n"
...
<main>
  ...
  <div class="mkt {% if macro_score < -1 %}bad{% endif %}" style="margin-bottom:10px">
    ...
    <span class="fpill {{ 'ok' if vix_cooling else 'warn' }}" style="margin-bottom:8px">
      {{ '✓ VIX Cooling' if vix_cooling else '⚠ VIX Rising' }}{% if vix_sma5 > 0 %} · SMA {{ vix_sma5 }}{% endif %}
    </span>
    ...
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:6px">
      <div>
        ...
        <div style="font-size:11px;color:#a78bfa;margin-top:2px">
          {%- if s.get('fvg_bonus',0) > 0 %}<span style="background:#a78bfa18;border:1px solid #a78bfa44;border-radius:4px;padding:1px 6px;margin-right:5px">🧲 FVG +{{ s.get('fvg_bonus',0) }}</span>{% endif -%}
          {%- if s.get('rs_bonus',0) > 0 %}<span style="background:#fbbf2418;border:1px solid #fbbf2444;border-radius:4px;padding:1px 6px;color:#fbbf24">🔄 RS Lag +{{ s.get('rs_bonus',0) }}</span>{% endif -%}
        </div>
      </div>
      <div style="text-align:right">
...
def dashboard():
    ...
    return render_template_string(DASH,
        ...
        vix_cooling=state["macro_intel"].get("vix_cooling", False),
        vix_sma5=round(state["macro_intel"].get("vix_sma5", 0.0), 1),
