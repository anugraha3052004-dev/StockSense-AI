...
FALLBACK_SYMBOLS = [
    ...
    "VEDL.NS","GRASIM.NS","BRITANNIA.NS","DABUR.NS","MARICO.NS",
    "GODREJCP.NS","PIDILITIND.NS","BERGEPAINT.NS","HAVELLS.NS","VOLTAS.NS",
    "CONCOR.NS","DELHIVERY.NS","APOLLOTYRE.NS","CEATLTD.NS","MRF.NS",
    "BALKRISIND.NS","MOTHERSON.NS","BOSCHLTD.NS","BHARATFORG.NS","BANKBARODA.NS",
    "CANBK.NS","PNB.NS","UNIONBANK.NS","FEDERALBNK.NS","IDFCFIRSTB.NS",
    "BANDHANBNK.NS","CHOLAFIN.NS","SHRIRAMFIN.NS","MUTHOOTFIN.NS","MPHASIS.NS",
    "COFORGE.NS","PERSISTENT.NS","LTTS.NS","KPIT.NS","DIXON.NS",
    "ZOMATO.NS","NAUKRI.NS","INDIAMART.NS","HAL.NS","BEL.NS",
    "RVNL.NS","IRFC.NS","DLF.NS","GODREJPROP.NS","PRESTIGE.NS",
    "APOLLO.NS","FORTIS.NS","METROPOLIS.NS","LALPATHLAB.NS","ITC.NS",
]
...
def fetch_nse_symbols():
    try:
        add_log("Fetching NSE symbol list ...")
        headers = {"User-Agent": "Mozilla/5.0"}
        resp = requests.get(NSE_CSV_URL, headers=headers, timeout=30)
        resp.raise_for_status()
        df = pd.read_csv(StringIO(resp.text))
        syms = [f"{s.strip()}.NS" for s in df["SYMBOL"].dropna().tolist()]
        add_log(f"Got {len(syms)} NSE symbols")
        return syms
    except Exception as e:
        add_log(f"NSE CSV failed ({e}) - using fallback")
...
def check_sector_momentum():
    ...
    for sector, stocks in SECTOR_MAP.items():
        ...
        scores = []
        try:
            raw = yf.download(" ".join(sample), period="20d", interval="1d",
