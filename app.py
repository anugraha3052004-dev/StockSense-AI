...
Scan schedule (IST):
  ...
  Daily rpt : 15:45 — today's stocks, P&L, what hit/missed, what went wrong
  Weekly rpt: Friday 16:00 — full week summary + sector breakdown
  Monthly   : Last Friday 16:15 — month totals, outcome + sector analysis
...
# ═══════════════════════════════════════════════════════════════════════════════
# PERFORMANCE ANALYTICS — DAILY · WEEKLY · MONTHLY  (ADD-ON)
# Zero changes to existing scan / score / email logic.
#
#  DAILY   -> 15:45 IST every market day  — today's stocks, what hit/missed
#  WEEKLY  -> Friday 16:00 IST            — full week + what went wrong
#  MONTHLY -> Last Friday of month 16:15  — month breakdown by sector
# ═══════════════════════════════════════════════════════════════════════════════
...
def _send_report_email(subject, html, plain):
    resend_key = os.getenv("RESEND_API_KEY", "")
    sender     = os.getenv("ALERT_EMAIL_SENDER", "")
    recips     = [r.strip() for r in os.getenv("ALERT_EMAIL_RECIPIENTS", "").split(",") if r.strip()]
    if not resend_key or not sender or not recips:
        add_log("Report email: config missing"); return False
    try:
        resp = requests.post(
        ...
        add_log(f"Report email error: {e}"); return False


def _wrap_html(title, subtitle, accent, body):
    return (
        ...
        f"<tr><td style='background:#0D1F2D;border-radius:10px 10px 0 0;"
        f"padding:26px 24px;border-bottom:3px solid {accent}'>"
        f"<div style='font-size:10px;color:{accent};letter-spacing:2px;"
        f"font-weight:700;margin-bottom:6px'>STOCKSENSE AI &middot; PERFORMANCE REPORT</div>"
        f"<div style='font-size:24px;font-weight:800;color:#FFFFFF'>{title}</div>"
        f"<div style='font-size:12px;color:#546E7A;margin-top:6px'>{subtitle}</div>"
        "</td></tr>"
        f"<tr><td style='background:#060E18;padding:16px 24px'>{body}</td></tr>"
        "<tr><td style='background:#0D1F2D;border-radius:0 0 10px 10px;"
        "padding:14px 24px;border-top:1px solid #1E3248'>"
        "<div style='font-size:11px;color:#37474F'>Automated report &mdash; not financial advice.</div>"
        "</td></tr>"
        "</table></td></tr></table></body></html>"
    )


def _scorecard_html(total, wins, losses, avg_pnl, t1c, t2c, slc,
                    best_sym, best_pnl, worst_sym, worst_pnl):
    win_rate = round(wins / total * 100) if total else 0
    wrc = "#00C853" if win_rate >= 60 else ("#FF9800" if win_rate >= 40 else "#FF1744")
    pc  = "#00C853" if avg_pnl >= 0 else "#FF1744"
    return (
        ...
        f"<div style='font-size:26px;font-weight:900;color:{wrc}'>{win_rate}%</div>"
        f"<div style='font-size:10px;color:#37474F'>{wins}W / {losses}L of {total}</div></div>"
        "<div style='background:#0D1F2D;border-radius:8px;padding:12px 6px'>"
        "<div style='font-size:9px;color:#546E7A;letter-spacing:1px;margin-bottom:4px'>AVG P&amp;L</div>"
        f"<div style='font-size:26px;font-weight:900;color:{pc}'>{avg_pnl:+.2f}%</div>"
        "<div style='font-size:10px;color:#37474F'>per trade</div></div>"
        "<div style='background:#0D1F2D;border-radius:8px;padding:12px 6px'>"
        "<div style='font-size:9px;color:#546E7A;letter-spacing:1px;margin-bottom:4px'>TARGETS</div>"
        f"<div style='font-size:26px;font-weight:900;color:#00C853'>{t1c}</div>"
        f"<div style='font-size:10px;color:#37474F'>T1 &nbsp;&middot;&nbsp; {t2c} T2</div></div>"
        "<div style='background:#0D1F2D;border-radius:8px;padding:12px 6px'>"
        "<div style='font-size:9px;color:#546E7A;letter-spacing:1px;margin-bottom:4px'>SL HIT</div>"
        f"<div style='font-size:26px;font-weight:900;color:{'#FF1744' if slc else '#00C853'}'>{slc}</div>"
        "<div style='font-size:10px;color:#37474F'>stop losses</div></div>"
        "</div>"
        "<div style='padding:10px 14px;background:#0D1F2D;border-radius:8px;"
        "display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px;"
        "font-size:11px;color:#546E7A'>"
        f"<span>&#127942; Best: <strong style='color:#00C853'>{best_sym} {best_pnl:+.2f}%</strong></span>"
        f"<span>&#128201; Worst: <strong style='color:#FF1744'>{worst_sym} {worst_pnl:+.2f}%</strong></span>"
        "</div>"
    )


def _sector_table_html(all_stocks):
    from collections import defaultdict
    sd = defaultdict(lambda: {"wins": 0, "total": 0, "pnl": []})
    for s in all_stocks:
        sec = s.get("sector", "OTHER")
        sd[sec]["total"] += 1
        sd[sec]["pnl"].append(s["pnl_pct"])
        if s["pnl_pct"] > 0:
            sd[sec]["wins"] += 1
    if not sd:
        return ""
    rows = ""
    for sec, d in sorted(sd.items(), key=lambda x: -(x[1]["wins"] / max(x[1]["total"], 1))):
        wr  = round(d["wins"] / d["total"] * 100) if d["total"] else 0
        avg = round(sum(d["pnl"]) / len(d["pnl"]), 2)
        wrc = "#00C853" if wr >= 60 else ("#FF9800" if wr >= 40 else "#FF1744")
        pc  = "#00C853" if avg >= 0 else "#FF1744"
        rows += (
            f"<tr style='border-bottom:1px solid #1E3248'>"
            f"<td style='padding:8px 12px;color:#E8F4FD;font-size:12px'>{sec}</td>"
            f"<td style='padding:8px 12px;text-align:center;color:#546E7A;font-size:12px'>{d['total']}</td>"
            f"<td style='padding:8px 12px;text-align:center;font-weight:700;color:{wrc};font-size:12px'>{wr}%</td>"
            f"<td style='padding:8px 12px;text-align:center;font-weight:700;color:{pc};font-size:12px'>{avg:+.2f}%</td>"
            f"</tr>"
        )
    return (
        "<table style='width:100%;border-collapse:collapse;background:#0D1F2D;"
        "border-radius:8px;overflow:hidden;margin-top:14px'>"
        "<tr style='background:#112233'>"
        "<th style='padding:9px 12px;text-align:left;font-size:10px;color:#546E7A'>SECTOR</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>TRADES</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>WIN RATE</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>AVG P&amp;L</th>"
        f"</tr>{rows}</table>"
    )


def _wrong_html(all_stocks):
    from collections import Counter
    wrongs = [w for s in all_stocks if s.get("pnl_pct", 0) <= 0
              for w in s.get("what_went_wrong", [])]
    if not wrongs:
        return ""
    counts = Counter(wrongs)
    items = "".join(
        f"<div style='display:flex;justify-content:space-between;align-items:center;"
        f"padding:8px 12px;border-bottom:1px solid #1E3248'>"
        f"<span style='font-size:11px;color:#CFD8DC;line-height:1.5'>{r}</span>"
        f"<span style='font-size:11px;font-weight:700;color:#FF9800;margin-left:12px;"
        f"white-space:nowrap'>{c}x</span></div>"
        for r, c in counts.most_common()
    )
    return (
        "<div style='background:#0D1F2D;border-radius:8px;overflow:hidden;margin-top:14px'>"
        "<div style='background:#1A0A0A;padding:10px 14px;"
        "font-size:10px;color:#FF9800;letter-spacing:1px;font-weight:700'>"
        "&#9888; WHAT WENT WRONG &mdash; LOSING TRADES</div>"
        f"{items}</div>"
    )


def _outcome_table_html(all_stocks):
    from collections import Counter
    counts = Counter(s["outcome"] for s in all_stocks)
    colors = {
        "TARGET 2 \u2705": "#00C853", "TARGET 1 \u2705": "#66BB6A",
        "T1 then SL \u26a0": "#FF9800", "STOP LOSS \u274c": "#FF1744",
        "PROFIT (open)": "#4CAF50", "LOSS (open)": "#EF5350",
    }
    total = len(all_stocks)
    rows  = ""
    for outcome, cnt in sorted(counts.items(), key=lambda x: -x[1]):
        pct = round(cnt / total * 100) if total else 0
        col = colors.get(outcome, "#546E7A")
        rows += (
            f"<tr style='border-bottom:1px solid #1E3248'>"
            f"<td style='padding:8px 12px;color:{col};font-size:12px;font-weight:700'>{outcome}</td>"
            f"<td style='padding:8px 12px;text-align:center;color:#E8F4FD;font-size:12px'>{cnt}</td>"
            f"<td style='padding:8px 12px;text-align:center;color:#546E7A;font-size:12px'>{pct}%</td>"
            f"</tr>"
        )
    return (
        "<table style='width:100%;border-collapse:collapse;background:#0D1F2D;"
        "border-radius:8px;overflow:hidden;margin-top:14px'>"
        "<tr style='background:#112233'>"
        "<th style='padding:9px 12px;text-align:left;font-size:10px;color:#546E7A'>OUTCOME</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>COUNT</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>% OF TRADES</th>"
        f"</tr>{rows}</table>"
    )
...
def fetch_eod_performance(positions_snapshot):
    ...
    for sym, pos in positions_snapshot.items():
        try:
            ...
            t1      = pos["t1"]; t2 = pos["t2"]; sl = pos["sl"]
            eod     = float(df_1d["Close"].iloc[-1])
            pnl     = round((eod - entry) / entry * 100, 2)
            d_high  = float(df_intra["High"].max())  if not df_intra.empty else eod
            d_low   = float(df_intra["Low"].min())   if not df_intra.empty else eod
            t1_hit  = d_high >= t1
            t2_hit  = d_high >= t2
            sl_hit  = d_low  <= sl
            ...
            if sl_hit and not t1_hit:
                wrong.append("Dropped below Stop Loss before reaching Target 1")
            if t1_hit and sl_hit:
                wrong.append("Hit Target 1 but reversed and hit Stop Loss — momentum failed")
            if not t1_hit and not sl_hit and pnl < 0:
                wrong.append(f"Stock drifted down {pnl:.2f}% — no strong move today")
            if not t1_hit and not sl_hit and pnl >= 0:
                wrong.append("Stock up but not enough to hit Target 1 yet — still holding")

            if t2_hit:
                outcome = "TARGET 2 \u2705"; oc = "#00C853"
            elif t1_hit and not sl_hit:
                outcome = "TARGET 1 \u2705"; oc = "#66BB6A"
            elif sl_hit and not t1_hit:
                outcome = "STOP LOSS \u274c"; oc = "#FF1744"
            elif sl_hit and t1_hit:
                outcome = "T1 then SL \u26a0"; oc = "#FF9800"
            elif pnl > 0:
                outcome = "PROFIT (open)"; oc = "#4CAF50"
            else:
                outcome = "LOSS (open)"; oc = "#EF5350"

            results.append({
                "symbol": pos["display"], "sector": pos.get("sector", "—"),
                "entry": entry, "t1": t1, "t2": t2, "sl": sl,
                "eod_close": round(eod, 2), "pnl_pct": pnl,
                "day_high": round(d_high, 2), "day_low": round(d_low, 2),
                "t1_hit": t1_hit, "t2_hit": t2_hit, "sl_hit": sl_hit,
                "outcome": outcome, "outcome_color": oc,
                "what_went_wrong": wrong,
                "alerted_at": pos.get("alerted_at", "—"),
            })
        except Exception as e:
            log.debug(f"EOD fetch {sym}: {e}")
    return results


def _stock_card_html(p):
    sign  = "+" if p["pnl_pct"] >= 0 else ""
    pc    = "#00C853" if p["pnl_pct"] > 0 else "#FF1744"
    t1c   = "#00C853" if p["t1_hit"] else "#455A64"
    t2c   = "#00C853" if p["t2_hit"] else "#455A64"
    slc   = "#FF1744" if (p["sl_hit"] and not p["t1_hit"]) else ("#FF9800" if p["sl_hit"] else "#455A64")
    t1l   = "HIT \u2713" if p["t1_hit"] else "missed"
    t2l   = "HIT \u2713" if p["t2_hit"] else "missed"
    sll   = "HIT \u2717" if (p["sl_hit"] and not p["t1_hit"]) else ("partial \u2717" if p["sl_hit"] else "safe \u2713")
    wrong = ""
    if p["what_went_wrong"] and not p["t2_hit"]:
        items = "".join(
            f"<div style='display:flex;gap:8px;margin-bottom:5px'>"
            f"<span style='color:#FF9800;font-size:13px'>&#9888;</span>"
            f"<span style='font-size:11px;color:#B0BEC5;line-height:1.5'>{w}</span></div>"
            for w in p["what_went_wrong"]
        )
        wrong = (
            "<div style='background:#FF980008;border-left:3px solid #FF9800;"
            "border-radius:0 6px 6px 0;padding:10px 14px;margin:10px 18px'>"
            "<div style='font-size:9px;color:#FF9800;font-weight:700;"
            f"letter-spacing:1px;margin-bottom:8px'>WHAT WENT WRONG</div>{items}</div>"
        )
    return (
        f"<div style='background:#0D1F2D;border:1px solid #1E3248;border-radius:10px;"
        f"margin-bottom:14px;overflow:hidden;border-left:4px solid {p['outcome_color']}'>"
        f"<div style='background:#112233;padding:13px 18px;display:flex;"
        f"justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px'>"
        f"<div><span style='font-size:17px;font-weight:800;color:#fff'>{p['symbol']}</span>"
        f"<span style='font-size:11px;color:#546E7A;margin-left:8px'>{p['sector']}</span>"
        f"<span style='font-size:10px;color:#37474F;margin-left:8px'>alerted {p['alerted_at']}</span></div>"
        f"<div style='text-align:right'>"
        f"<div style='font-size:11px;color:#546E7A'>OUTCOME</div>"
        f"<div style='font-size:12px;font-weight:700;color:{p['outcome_color']}'>{p['outcome']}</div>"
        f"</div></div>"
        f"<div style='padding:10px 18px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px'>"
        f"<div style='background:#0A1929;border-radius:6px;padding:8px 10px'>"
        f"<div style='font-size:9px;color:#546E7A'>ENTRY</div>"
        f"<div style='font-size:13px;font-weight:700;color:#E8F4FD'>Rs.{p['entry']:,.2f}</div></div>"
        f"<div style='background:#0A1929;border-radius:6px;padding:8px 10px'>"
        f"<div style='font-size:9px;color:#546E7A'>EOD CLOSE</div>"
        f"<div style='font-size:13px;font-weight:700;color:#E8F4FD'>Rs.{p['eod_close']:,.2f}</div></div>"
        f"<div style='background:#0A1929;border-radius:6px;padding:8px 10px'>"
        f"<div style='font-size:9px;color:#546E7A'>P&amp;L</div>"
        f"<div style='font-size:16px;font-weight:900;color:{pc}'>{sign}{p['pnl_pct']:.2f}%</div></div>"
        f"</div>"
        f"<div style='padding:4px 18px 10px;display:grid;grid-template-columns:repeat(3,1fr);gap:7px'>"
        f"<div style='background:#0A1520;border:1px solid {t1c}44;border-radius:5px;"
        f"padding:6px 8px;text-align:center'>"
        f"<div style='font-size:9px;color:#546E7A'>T1 Rs.{p['t1']:,.0f}</div>"
        f"<div style='font-size:11px;font-weight:700;color:{t1c}'>{t1l}</div></div>"
        f"<div style='background:#0A1520;border:1px solid {t2c}44;border-radius:5px;"
        f"padding:6px 8px;text-align:center'>"
        f"<div style='font-size:9px;color:#546E7A'>T2 Rs.{p['t2']:,.0f}</div>"
        f"<div style='font-size:11px;font-weight:700;color:{t2c}'>{t2l}</div></div>"
        f"<div style='background:#0A1520;border:1px solid {slc}44;border-radius:5px;"
        f"padding:6px 8px;text-align:center'>"
        f"<div style='font-size:9px;color:#546E7A'>SL Rs.{p['sl']:,.0f}</div>"
        f"<div style='font-size:11px;font-weight:700;color:{slc}'>{sll}</div></div>"
        f"</div>"
        f"<div style='padding:2px 18px 8px;font-size:10px;color:#37474F'>"
        f"Day High Rs.{p['day_high']:,.2f} &nbsp;&middot;&nbsp; Day Low Rs.{p['day_low']:,.2f}"
        f"</div>{wrong}</div>"
    )


def send_daily_report_email(perf_list, date_str):
    if not perf_list:
        add_log("Daily report: no positions today"); return False
    total   = len(perf_list)
    wins    = sum(1 for p in perf_list if p["pnl_pct"] > 0)
    losses  = sum(1 for p in perf_list if p["pnl_pct"] <= 0)
    t1c     = sum(1 for p in perf_list if p["t1_hit"])
    t2c     = sum(1 for p in perf_list if p["t2_hit"])
    slc     = sum(1 for p in perf_list if p["sl_hit"] and not p["t1_hit"])
    avg_pnl = round(sum(p["pnl_pct"] for p in perf_list) / total, 2)
    ...
    subject = (
        ...
        f"Win {win_rate}% | {wins}W {losses}L | Avg {avg_pnl:+.2f}% | StockSense AI"
    )
    scorecard  = _scorecard_html(total, wins, losses, avg_pnl, t1c, t2c, slc,
                                 best["symbol"], best["pnl_pct"], worst["symbol"], worst["pnl_pct"])
    cards      = "".join(_stock_card_html(p) for p in perf_list)
    body       = (
        f"<div style='background:#0A1929;border-radius:10px;padding:18px;margin-bottom:16px'>"
        f"{scorecard}</div>"
        f"<div style='font-size:10px;color:#546E7A;letter-spacing:2px;margin-bottom:12px'>"
        f"TODAY'S STOCK PERFORMANCE</div>{cards}"
    )
    html  = _wrap_html(f"\U0001f4ca Daily Report \u2014 {date_str}",
                       f"{total} stock{'s' if total!=1 else ''} alerted today",
                       "#00E5FF", body)
    plain  = f"STOCKSENSE AI \u2014 DAILY REPORT\n{date_str}\n{'='*50}\n\n"
    plain += f"Stocks:{total}  Wins:{wins}  Losses:{losses}  Win Rate:{win_rate}%\n"
    plain += f"Avg P&L:{avg_pnl:+.2f}%  T1:{t1c}  T2:{t2c}  SL:{slc}\n"
    plain += f"Best:{best['symbol']} {best['pnl_pct']:+.2f}%  Worst:{worst['symbol']} {worst['pnl_pct']:+.2f}%\n\n"
    for p in perf_list:
        sign = "+" if p["pnl_pct"] >= 0 else ""
        plain += f"{p['symbol']} [{p['sector']}] \u2014 {p['outcome']}\n"
        plain += f"  Entry Rs.{p['entry']:,.2f} -> Close Rs.{p['eod_close']:,.2f} | P&L {sign}{p['pnl_pct']:.2f}%\n"
        plain += f"  T1:{'HIT' if p['t1_hit'] else 'missed'} T2:{'HIT' if p['t2_hit'] else 'missed'} SL:{'HIT' if p['sl_hit'] else 'safe'}\n"
        plain += f"  Day High Rs.{p['day_high']:,.2f} | Day Low Rs.{p['day_low']:,.2f}\n"
        for w in p["what_went_wrong"]:
            plain += f"  -> {w}\n"
        plain += "\n"
    plain += "Not financial advice.\n"
    ok = _send_report_email(subject, html, plain)
    add_log(f"Daily report {'sent' if ok else 'failed'} | {total} stocks | {win_rate}% win")
    return ok
...
def run_eod_analytics():
    ...
    add_log(f"=== Daily Analytics [{date_str}] ===")
    positions_snapshot = dict(state["active_positions"])
    ...
    for ep in state["exited_positions"]:
        ...
        if sym_key not in positions_snapshot and today_label in ep.get("exited_at", ""):
            positions_snapshot[sym_key] = {
                "symbol": sym_key, "display": ep["symbol"],
                "entry_price": ep["entry"],
                "sl":  ep["entry"] * (1 - SL_PCT / 100),
    ...
    if not positions_snapshot:
        add_log("Daily analytics: no positions today"); return
    perf_list = fetch_eod_performance(positions_snapshot)
    if not perf_list:
        add_log("Daily analytics: price fetch failed"); return
    send_daily_report_email(perf_list, date_str)
    history = load_analytics_history()
    history.append({
        "date": date_str, "total": len(perf_list),
        "wins":  sum(1 for p in perf_list if p["pnl_pct"] > 0),
        "losses":sum(1 for p in perf_list if p["pnl_pct"] <= 0),
        ...
        "stocks": [{
            ...
            "pnl_pct": p["pnl_pct"], "outcome": p["outcome"],
            "t1_hit": p["t1_hit"], "t2_hit": p["t2_hit"], "sl_hit": p["sl_hit"],
            "what_went_wrong": p["what_went_wrong"],
        } for p in perf_list],
    })
    save_analytics_history(history)
    add_log("=== Daily analytics done ===")


# ════════════════════════════════════════════════════════════════════════════════
# WEEKLY REPORT — Every Friday 16:00 IST
# ════════════════════════════════════════════════════════════════════════════════

def _week_data():
    cutoff = datetime.now(IST) - timedelta(days=7)
    out = []
    for d in load_analytics_history():
        try:
            if datetime.strptime(d["date"], "%d %b %Y") >= cutoff:
                out.append(d)
        except Exception:
            pass
    return out
...
def send_weekly_report_email():
    week = _week_data()
    if not week:
        add_log("Weekly report: no data this week"); return False
    now_ist  = datetime.now(IST)
    date_str = now_ist.strftime("%d %b %Y")
    all_s    = [s for d in week for s in d.get("stocks", [])]
    total    = sum(d["total"]  for d in week)
    wins     = sum(d["wins"]   for d in week)
    losses   = sum(d["losses"] for d in week)
    t1c      = sum(d.get("t1_count", 0) for d in week)
    t2c      = sum(d.get("t2_count", 0) for d in week)
    slc      = sum(d.get("sl_count", 0) for d in week)
    avg_pnl  = round(sum(d["avg_pnl"] * d["total"] for d in week) / total, 2) if total else 0
    win_rate = round(wins / total * 100) if total else 0
    best_s   = max(all_s, key=lambda x: x["pnl_pct"]) if all_s else {"symbol": "—", "pnl_pct": 0}
    worst_s  = min(all_s, key=lambda x: x["pnl_pct"]) if all_s else {"symbol": "—", "pnl_pct": 0}

    subject = (
        f"\U0001f4c5 Weekly Report | Win {win_rate}% | "
        f"{wins}W {losses}L of {total} | Avg {avg_pnl:+.2f}% | StockSense AI"
    )
    scorecard = _scorecard_html(total, wins, losses, avg_pnl, t1c, t2c, slc,
                                best_s["symbol"], best_s["pnl_pct"],
                                worst_s["symbol"], worst_s["pnl_pct"])
    ...
    for d in week:
        ...
        avg = d.get("avg_pnl", 0)
        pc  = "#00C853" if avg >= 0 else "#FF1744"
        syms = ", ".join(s["symbol"] for s in d.get("stocks", []))[:55]
        day_rows += (
            f"<tr style='border-bottom:1px solid #1E3248'>"
            f"<td style='padding:8px 12px;color:#E8F4FD;font-size:12px'>{d['date']}</td>"
            f"<td style='padding:8px 12px;text-align:center;color:#546E7A;font-size:12px'>{d['total']}</td>"
            f"<td style='padding:8px 12px;text-align:center;font-weight:700;color:{wrc};font-size:12px'>{wr}%</td>"
            f"<td style='padding:8px 12px;text-align:center;font-weight:700;color:{pc};font-size:12px'>{avg:+.2f}%</td>"
            f"<td style='padding:8px 12px;color:#546E7A;font-size:11px'>{syms}</td>"
            f"</tr>"
        )
    day_table = (
        "<table style='width:100%;border-collapse:collapse;background:#0D1F2D;"
        "border-radius:8px;overflow:hidden;margin-top:14px'>"
        "<tr style='background:#112233'>"
        "<th style='padding:9px 12px;text-align:left;font-size:10px;color:#546E7A'>DATE</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>TRADES</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>WIN %</th>"
        "<th style='padding:9px 12px;text-align:center;font-size:10px;color:#546E7A'>AVG P&amp;L</th>"
        f"<th style='padding:9px 12px;text-align:left;font-size:10px;color:#546E7A'>STOCKS</th>"
        f"</tr>{day_rows}</table>"
    )
    body = (
        f"<div style='background:#0A1929;border-radius:10px;padding:18px;margin-bottom:16px'>"
        f"<div style='font-size:10px;color:#7C4DFF;letter-spacing:2px;margin-bottom:12px;"
        f"font-weight:700'>THIS WEEK \u2014 {len(week)} TRADING DAYS</div>{scorecard}</div>"
        f"<div style='font-size:10px;color:#546E7A;letter-spacing:2px;margin-bottom:4px'>DAY BY DAY</div>"
        f"{day_table}"
        f"<div style='font-size:10px;color:#546E7A;letter-spacing:2px;margin-top:16px;margin-bottom:4px'>"
        f"SECTOR PERFORMANCE</div>{_sector_table_html(all_s)}{_wrong_html(all_s)}"
    )
    html  = _wrap_html("\U0001f4c5 Weekly Performance Report",
                       f"Week ending {date_str} \u00b7 {total} trades across {len(week)} days",
                       "#7C4DFF", body)
    plain  = f"STOCKSENSE AI \u2014 WEEKLY REPORT\nWeek ending {date_str}\n{'='*50}\n\n"
    plain += f"Trades:{total} Wins:{wins} Losses:{losses} Win Rate:{win_rate}%\n"
    plain += f"Avg P&L:{avg_pnl:+.2f}% T1:{t1c} T2:{t2c} SL:{slc}\n"
    for d in week:
        plain += f"  {d['date']}: {d['total']} trades | {d['win_rate']}% win | avg {d['avg_pnl']:+.2f}%\n"
    plain += "\nNot financial advice.\n"
    ok = _send_report_email(subject, html, plain)
    add_log(f"Weekly report {'sent' if ok else 'failed'} | {total} trades | {win_rate}% win")
    return ok


def run_weekly_report():
    add_log("=== Weekly Report starting ===")
    send_weekly_report_email()
    add_log("=== Weekly report done ===")


# ════════════════════════════════════════════════════════════════════════════════
# MONTHLY REPORT — Last Friday of month 16:15 IST
# ════════════════════════════════════════════════════════════════════════════════

def _is_last_friday_of_month():
...
def send_monthly_report_email():
    month = _month_data()
    if not month:
        add_log("Monthly report: no data this month"); return False
    now_ist   = datetime.now(IST)
    month_str = now_ist.strftime("%B %Y")
    all_s     = [s for d in month for s in d.get("stocks", [])]
    total     = sum(d["total"]  for d in month)
    wins      = sum(d["wins"]   for d in month)
    losses    = sum(d["losses"] for d in month)
    t1c       = sum(d.get("t1_count", 0) for d in month)
    t2c       = sum(d.get("t2_count", 0) for d in month)
    slc       = sum(d.get("sl_count", 0) for d in month)
    avg_pnl   = round(sum(d["avg_pnl"] * d["total"] for d in month) / total, 2) if total else 0
    win_rate  = round(wins / total * 100) if total else 0
    best_s    = max(all_s, key=lambda x: x["pnl_pct"]) if all_s else {"symbol": "—", "pnl_pct": 0}
    worst_s   = min(all_s, key=lambda x: x["pnl_pct"]) if all_s else {"symbol": "—", "pnl_pct": 0}

    subject = (
        f"\U0001f4c6 Monthly Report {month_str} | Win {win_rate}% | "
        f"{wins}W {losses}L of {total} | Avg {avg_pnl:+.2f}% | StockSense AI"
    )
    scorecard = _scorecard_html(total, wins, losses, avg_pnl, t1c, t2c, slc,
                                best_s["symbol"], best_s["pnl_pct"],
    ...
    for d in month:
        ...
        day_rows += (
            f"<tr style='border-bottom:1px solid #1E3248'>"
            f"<td style='padding:7px 12px;color:#E8F4FD;font-size:11px'>{d['date']}</td>"
            f"<td style='padding:7px 12px;text-align:center;color:#546E7A;font-size:11px'>{d['total']}</td>"
            f"<td style='padding:7px 12px;text-align:center;font-weight:700;color:{wrc};font-size:11px'>{wr}%</td>"
            f"<td style='padding:7px 12px;text-align:center;font-weight:700;color:{pc};font-size:11px'>{avg:+.2f}%</td>"
            f"</tr>"
        )
    day_table = (
        "<table style='width:100%;border-collapse:collapse;background:#0D1F2D;"
        "border-radius:8px;overflow:hidden;margin-top:14px'>"
        "<tr style='background:#112233'>"
        "<th style='padding:8px 12px;text-align:left;font-size:10px;color:#546E7A'>DATE</th>"
        "<th style='padding:8px 12px;text-align:center;font-size:10px;color:#546E7A'>TRADES</th>"
        "<th style='padding:8px 12px;text-align:center;font-size:10px;color:#546E7A'>WIN %</th>"
        f"<th style='padding:8px 12px;text-align:center;font-size:10px;color:#546E7A'>AVG P&amp;L</th>"
        f"</tr>{day_rows}</table>"
    )
    body = (
        f"<div style='background:#0A1929;border-radius:10px;padding:18px;margin-bottom:16px'>"
        f"<div style='font-size:10px;color:#FF9800;letter-spacing:2px;margin-bottom:12px;"
        f"font-weight:700'>{month_str.upper()} \u2014 {len(month)} TRADING DAYS \u00b7 {total} TRADES</div>"
        f"{scorecard}</div>"
        f"<div style='font-size:10px;color:#546E7A;letter-spacing:2px;margin-bottom:4px'>DAY BY DAY</div>"
        f"{day_table}"
        f"<div style='font-size:10px;color:#546E7A;letter-spacing:2px;margin-top:16px;margin-bottom:4px'>"
        f"TRADE OUTCOME BREAKDOWN</div>{_outcome_table_html(all_s)}"
        f"<div style='font-size:10px;color:#546E7A;letter-spacing:2px;margin-top:16px;margin-bottom:4px'>"
        f"SECTOR PERFORMANCE THIS MONTH</div>{_sector_table_html(all_s)}{_wrong_html(all_s)}"
    )
    html  = _wrap_html(f"\U0001f4c6 Monthly Report \u2014 {month_str}",
                       f"{total} trades across {len(month)} days | Full month breakdown",
                       "#FF9800", body)
    plain  = f"STOCKSENSE AI \u2014 MONTHLY REPORT\n{month_str}\n{'='*50}\n\n"
    plain += f"Trades:{total} Wins:{wins} Losses:{losses} Win Rate:{win_rate}%\n"
    plain += f"Avg P&L:{avg_pnl:+.2f}% T1:{t1c} T2:{t2c} SL:{slc}\n"
    for d in month:
        plain += f"  {d['date']}: {d['total']} trades | {d['win_rate']}% win | avg {d['avg_pnl']:+.2f}%\n"
    plain += "\nNot financial advice.\n"
    ok = _send_report_email(subject, html, plain)
    add_log(f"Monthly report {'sent' if ok else 'failed'} | {month_str} | {total} trades | {win_rate}% win")
    return ok


def run_monthly_report():
    if not _is_last_friday_of_month():
        add_log("Monthly report: not last Friday of month — skipping"); return
    add_log("=== Monthly Report starting ===")
    send_monthly_report_email()
    add_log("=== Monthly report done ===")
...
def start_scheduler():
    ...
    # Daily report  — 15:45 IST every market day
    scheduler.add_job(run_eod_analytics,  CronTrigger(hour=15, minute=45, day_of_week=WD, timezone=IST), id="eod_analytics")
    # Weekly report — every Friday 16:00 IST
    scheduler.add_job(run_weekly_report,  CronTrigger(hour=16, minute=0,  day_of_week="fri", timezone=IST), id="weekly_report")
    # Monthly report — every Friday 16:15 IST (function checks internally if it's last Friday)
    scheduler.add_job(run_monthly_report, CronTrigger(hour=16, minute=15, day_of_week="fri", timezone=IST), id="monthly_report")
    ...
    add_log(
        ...
        "DAILY REPORT: 15:45 | WEEKLY: Fri 16:00 | MONTHLY: Last Fri 16:15 | Mon-Fri"
    )
    return scheduler
