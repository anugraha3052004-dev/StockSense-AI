...
<table width="600" style="max-width:600px;width:100%">
  ...
  <tr><td style="background:#0B1826;border-radius:0 0 14px 14px;padding:18px 28px;border-top:1px solid #1A2B3C">
    ...
    try:
        ...
        add_log(f"✅ Email sent via SMTP → {recips}")
        return True
    except Exception as smtp_err:
        add_log(f"SMTP failed ({smtp_err}) — trying SSL port 465 …")
    ...
    try:
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=ctx) as srv:
            srv.login(sender, pwd)
            srv.sendmail(sender, recips, msg.as_string())
        add_log(f"✅ Email sent via SSL → {recips}")
        return True
    except Exception as ssl_err:
        add_log(f"SSL also failed ({ssl_err}) — trying SendGrid …")

    # Fallback: SendGrid HTTP API (set SENDGRID_API_KEY in Render env)
    ...
    if sg_key:
        try:
            ...
            resp = req.post(
                ...
                json={
                    "personalizations": [{"to": [{"email": r} for r in recips]}],
                    "from": {"email": sender},
                    "subject": subject,
                    "content": [
            ...
            if resp.status_code in (200, 202):
                add_log(f"✅ Email sent via SendGrid → {recips}")
                return True
            else:
                add_log(f"❌ SendGrid error: {resp.status_code} {resp.text[:200]}")
        except Exception as sg_err:
            add_log(f"❌ SendGrid failed: {sg_err}")
    else:
        add_log("❌ All SMTP attempts failed. Set SENDGRID_API_KEY in Render env vars.")
        add_log("   Get free key at: https://sendgrid.com (100 emails/day free)")
