...
# ── ENTRY QUALITY FILTERS (prevent chasing exhausted moves) ────────────────────
# FIX: stocks already at day-high or up 3%+ before 10AM are reversal risks
EQ_DAY_RANGE_MAX    = 0.80  # reject if price in top 20% of today's high-low range
EQ_ALREADY_MOVED    = 2.4   # reject if already moved 2x target (T1_PCT*2)
EQ_MAX_MOVE_PRE10   = 2.0   # before 10:00 IST: max allowed 1D move
EQ_MAX_MOVE_PRE11   = 3.0   # 10:00-11:00 IST: max allowed 1D move
# After 11:00 IST: normal P1_MAX_MOVE (7.0) applies
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for sym in batch:
            try:
                ...
                # ── FIX 1: Day Range Position — reject if stock near day high ──────────────
                # If stock is in top 20% of today's range it's in the exhaustion zone.
                # Smart money already bought at lower prices and is now selling to latecomers.
                if candle_range > 0:
                    day_pos = (curr - low_today) / candle_range
                    if day_pos > EQ_DAY_RANGE_MAX:
                        add_log(f"    SKIP {sym.replace('.NS','')} — day pos {day_pos:.0%} > 80% of range (near day high, reversal risk)")
                        continue

                # ── FIX 2: Time-aware move cap — opening spikes are not trends ──────────
                # Before 11 AM, a 3%+ move is usually opening euphoria, not a real trend.
                # The stock has already done most of its move for the day.
                _scan_hour = datetime.now(IST).hour
                if _scan_hour < 10 and chg > EQ_MAX_MOVE_PRE10:
                    add_log(f"    SKIP {sym.replace('.NS','')} — {chg:.1f}% before 10AM (opening spike, not trend)")
                    continue
                elif _scan_hour < 11 and chg > EQ_MAX_MOVE_PRE11:
                    add_log(f"    SKIP {sym.replace('.NS','')} — {chg:.1f}% before 11AM > {EQ_MAX_MOVE_PRE11}% limit (too extended early)")
                    continue

                # ── FIX 3: Already-moved check — move must leave room for target ─────────
                # If stock already moved 2x the target (>2.4%), the fuel is spent.
                # Buying now means chasing the trade at exactly the wrong time.
                if chg > EQ_ALREADY_MOVED:
                    add_log(f"    SKIP {sym.replace('.NS','')} — {chg:.1f}% already > 2x target {EQ_ALREADY_MOVED}% (move exhausted)")
                    continue
                # ───────────────────────────────────────────────────────────────────────────────
...
def phase2_analyze(candidates, market, scan_mode=SCAN_MODE_INTRADAY, ss=None):
    ...
    for c in candidates:
        ...
        hard_reject = False
        # ── FIX 4: RSI hard cap lowered 78 → 68 ───────────────────────────────────────
        # For a 1.2% profit target, RSI must have room to grow.
        # RSI 70+ on a stock already up 3%+ = exhaustion, not momentum.
        # 78 was far too lenient — GRANULES had RSI 70 and failed immediately.
        if rsi > 68:
            hard_reject = True
            add_log(f"    REJECT {sym.replace('.NS','')} — RSI {rsi:.1f} overbought (>68 hard cap for swing trades)")
        if cmp < e20 * 0.97:
            hard_reject = True
