...
Scan schedule (IST):
  Intraday  : 09:45 . 10:25 . 11:30 . 13:30 . 14:45
  Post-mkt  : 17:30 (next-day setup scan)
  Night     : 21:15
"""
...
# Each weight represents confidence contribution of that signal
S = {
    "breakout":  2.5,  # volume+price breakout — strongest signal
    "trend":     2.0,  # EMA alignment — confirms direction
    "macd":      1.5,  # momentum confirmation
    "rsi":       1.5,  # entry timing
    "volume":    1.0,  # buying pressure
    "bb":        1.0,  # position in range
    "candle":    0.5,  # candlestick pattern bonus
    "news":      0.5,  # positive news tailwind
}
MAX_SCORE            = sum(S.values())
MIN_SCORE            = 6.0   # must pass 6/10.5 — stricter quality bar
MIN_SCORE_BAD_MARKET = 7.5   # very high bar in bad market

# ── TARGET LEVELS (1-2% profit in 2-3 days) ──────────────────────────────────
ENTRY_BUF = 0.2   # tight entry band ±0.2%
T1_PCT    = 1.2   # T1 = +1.2% — first target, book 50%
T2_PCT    = 2.2   # T2 = +2.2% — second target, book rest
SL_PCT    = 0.8   # SL = -0.8% — tight stop, good R:R ~1.5
...
state = {
    ...
    "macro_intel": {          # latest macro intelligence result
...
# ═══════════════════════════════════════════════════════════════════════════════
# Sources  : Yahoo Finance (free) — S&P500, Nasdaq, VIX, Oil, Gold, USD/INR,
#            US 10Y yield, EM ETF, INDA ETF + yfinance news headlines
# Runs     : Before every scan as Filter 0 (pre-market gate)
# Output   : macro_score (-4 to +2) → adjusts MIN_SCORE or skips scan entirely
# ─────────────────────────────────────────────────────────────────────────────
# Score rules:
#   +2  → Very bullish  → MIN_SCORE lowered to 4.5 (catch more setups)
#   0   → Neutral       → Normal MIN_SCORE 5.0
#  -1   → Mild risk     → MIN_SCORE raised to 6.0
#  -2   → Moderate risk → MIN_SCORE raised to 7.0 + email warning
#  -3+  → High risk     → SKIP scan + send MACRO CAUTION email
# ═══════════════════════════════════════════════════════════════════════════════

# Macro thresholds
VIX_DANGER     = 25.0   # fear spike — global selloff risk
VIX_CALM       = 15.0   # calm market — good for swing trades
OIL_SPIKE_PCT  = 2.5    # oil up/dn >2.5% in a day = macro event
SP500_FALL_PCT = 1.0    # S&P fell >1% → India likely opens red
USDINR_WEAK    = 84.5   # rupee above this = FII selling pressure
TNX_SPIKE      = 0.15   # 10Y yield rose > 15bps in a day = bond stress

# Sector impact of macro events (used to boost/suppress sector-specific alerts)
OIL_BENEFICIARY_SECTORS  = {"ENERGY"}          # oil up → good
OIL_HURT_SECTORS         = {"AUTO","FMCG"}     # oil up → bad input cost
WEAK_RUPEE_HURT          = {"FMCG","AUTO"}     # import costs rise
WEAK_RUPEE_HELP          = {"IT"}              # dollar earners benefit
RATE_CUT_HELP            = {"BANK","FINANCE","REALTY"}  # cheaper money
RATE_CUT_HURT            = {}                  # none directly
RATE_HIKE_HURT           = {"BANK","FINANCE","REALTY"}
GLOBAL_SELLOFF_HURT      = set(["IT","METAL","ENERGY"])  # FII-heavy sectors


def fetch_macro_signal(ticker_sym, period="5d", interval="1d"):
    """Fetch a single macro ticker via yfinance. Returns (latest_val, chg_pct, ok)."""
    try:
...
def fetch_macro_news():
    """
    Fetch macro-relevant headlines from yfinance news on key tickers.
    Scans for high-impact keywords: rate cut/hike, war, sanctions,
    recession, GDP, inflation, RBI, Fed, default, crisis.
    Returns (sentiment: int [-2..+2], headlines: list, keywords_hit: list)
    """
...
def run_macro_intelligence():
    """
    Full macro check. Returns macro_context dict with:
      - macro_score      : int (-4 to +2) — overall market risk level
      - min_score_adj    : float — adjusted MIN_SCORE for this scan
      - skip_scan        : bool — True if macro risk is too high
      - signals          : list of human-readable signal strings
      - sector_boosts    : set of sectors to favour
      - sector_suppresses: set of sectors to avoid
      - summary          : one-line summary for email header
    """
    ...
    # ── 1. VIX — global fear gauge ────────────────────────────────────────────
    vix, vix_chg, vix_ok = fetch_macro_signal("^VIX")
    if vix_ok:
        if vix > VIX_DANGER:
            score -= 2
            signals.append(f"⚠ VIX {vix:.1f} — FEAR SPIKE (>{VIX_DANGER}) · Global selloff risk · All sectors under pressure")
    ...
    # ── 4. Crude Oil — direct India impact ────────────────────────────────────
    oil, oil_chg, oil_ok = fetch_macro_signal("CL=F")
    if oil_ok:
        if oil_chg >= OIL_SPIKE_PCT:
            score -= 1
            signals.append(f"⚠ Crude Oil +{oil_chg:.1f}% (${oil:.1f}) — Oil spike · Hurts AUTO/FMCG · Helps ENERGY")
    ...
    # ── 6. US 10Y Treasury Yield — rate stress ────────────────────────────────
    tnx, tnx_chg, tnx_ok = fetch_macro_signal("^TNX")
    if tnx_ok:
        tnx_abs_chg = abs(tnx_chg / 100 * tnx)  # approximate bps move
        if tnx_chg >= 2.0:   # yield rising fast
            score -= 1
            signals.append(f"⚠ US 10Y Yield {tnx:.2f}% (+{tnx_chg:.2f}%) — Rate stress · FIIs may exit India for US bonds")
        elif tnx_chg <= -2.0:  # yield falling = rate cut expectations
            score += 1
    ...
    # ── 8. Emerging Markets ETF — FII flow proxy ─────────────────────────────
    ...
    if score >= 2:
        ...
        min_score_adj = MIN_SCORE + 1.0
        skip_scan     = False
        summary       = f"Mild macro risk (score {score}) — raising bar to {min_score_adj:.1f}"
    elif score == -2:
        min_score_adj = MIN_SCORE + 2.0
        skip_scan     = False
        summary       = f"Moderate macro risk (score {score}) — high bar {min_score_adj:.1f}, caution email sent"
    else:  # <= -3
        min_score_adj = MIN_SCORE + 3.0
        skip_scan     = True
        summary       = f"HIGH MACRO RISK (score {score}) — scan skipped, caution email sent"

    # Remove contradictions (sector can't be both boost and suppress)
...
def send_macro_caution_email(macro):
    """Send a caution email when macro risk is too high to scan safely."""
...
<table width="600" style="max-width:600px;width:100%">
  ...
  <tr><td style="background:#0F1F2E;padding:24px 28px">
    ...
    <!-- Score banner -->
    <div style="background:#FF990015;border:1px solid #FF990044;border-radius:10px;padding:16px 20px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center">
      <div>
        ...
        <div style="font-size:12px;color:#78909C">out of -5 (max risk) to +2 (bullish)</div>
      </div>
      <div style="text-align:right">
        <div style="font-size:12px;color:#FF9900;font-weight:700">TODAY'S ACTION</div>
        <div style="font-size:16px;color:#FF1744;font-weight:800">{"AVOID NEW TRADES" if score <= -3 else "HIGH CAUTION"}</div>
        <div style="font-size:11px;color:#607D8B;margin-top:4px">Protect existing positions</div>
      </div>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:16px">
      <tr style="background:#1e3248">
        <td style="padding:8px 12px;color:#607D8B;font-size:12px">VIX</td>
        <td style="padding:8px 12px;text-align:right;color:#E8F4FD;font-weight:700">{macro.get("vix","--")}</td>
      </tr>
      <tr>
        <td style="padding:8px 12px;color:#607D8B;font-size:12px;border-bottom:1px solid #1e3248">S&P 500 Change</td>
        <td style="padding:8px 12px;text-align:right;font-weight:700;border-bottom:1px solid #1e3248;color:{"#FF1744" if macro.get("sp500_chg",0)<0 else "#00C853"}">{macro.get("sp500_chg",0):+.2f}%</td>
      </tr>
      <tr>
        <td style="padding:8px 12px;color:#607D8B;font-size:12px;border-bottom:1px solid #1e3248">Crude Oil</td>
        <td style="padding:8px 12px;text-align:right;color:#E8F4FD;font-weight:700;border-bottom:1px solid #1e3248">${macro.get("oil",0):.1f}</td>
      </tr>
      <tr>
        <td style="padding:8px 12px;color:#607D8B;font-size:12px">USD/INR</td>
        <td style="padding:8px 12px;text-align:right;color:#E8F4FD;font-weight:700">{macro.get("usdinr",0):.2f}</td>
      </tr>
    </table>

    <!-- Bear news -->
    {"" if not macro["bear_keywords"] else f'<div style="background:#FF174410;border:1px solid #FF174433;border-radius:8px;padding:12px 16px"><div style="font-size:10px;color:#FF1744;letter-spacing:1px;text-transform:uppercase;margin-bottom:6px">Risk Keywords Detected</div><div style="color:#FF6B6B;font-size:13px">{bear_html}</div></div>'}

    <div style="margin-top:16px;background:#1e3248;border-radius:8px;padding:14px 16px;text-align:center">
      <div style="color:#FFD600;font-weight:700;font-size:14px">Suggested Action</div>
      <div style="color:#B0BEC5;font-size:12px;margin-top:4px">
        {"Do not enter new trades today. Tighten stops on existing positions." if score <= -3 else "Only trade highest-conviction setups (score 7+). Keep position sizes small."}
      </div>
    </div>
  </td></tr>
...
def phase1_scan(symbols, strong_sectors):
    """
    Phase 1 — Quick broad filter across all 413 stocks.
    Uses 10 days of daily OHLCV data downloaded in batches.

    Filters applied (ALL must pass):
      1. Price >= Rs.50 — removes penny/illiquid stocks
      2. Price move 0.5-7% today — moving but not parabolic
      3. Volume >= 1.5x 10-day average — genuine buying interest
      4. SMA5 > SMA10 — short-term uptrend confirmed
      5. Not a gap-down recovery (open must be above prev close)

    Additional quality score for sorting:
      - Strong sector bonus
      - Higher volume ratio ranked first
    """
    candidates = []
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for sym in batch:
            try:
                ...
                # ── Filter 2: Daily price move range ─────────────────────
                chg = (curr - prev) / prev * 100 if prev > 0 else 0
                if not (P1_MIN_MOVE <= chg <= P1_MAX_MOVE): continue

                # ── Filter 3: Not a gap-down bounce ──────────────────────
                # Reject if stock gapped down at open (open < prev close by >0.5%)
                ...
                # ── Filter 5: Short-term trend (SMA5 > SMA10) ────────────
                sma5  = float(close.rolling(5).mean().iloc[-1])
                sma10 = float(close.rolling(10).mean().iloc[-1])
                if curr < sma5 or sma5 < sma10: continue

                # ── Filter 6: Not near 52-week high risk zone ─────────────
                # Avoid stocks already up 15%+ in last 10 days (parabolic)
                low10d = float(close.iloc[-10:].min())
                rally_10d = (curr - low10d) / low10d * 100 if low10d > 0 else 0
                if rally_10d > 20: continue

                # ── Filter 7: Body size check (not doji/spinning top) ────
                # Candle body should be at least 30% of the range — real move
                ...
                # Quality score for sorting (higher = better candidate)
    ...
    # Sort by quality score — strong sector + high volume first
...
def phase2_analyze(candidates, market, scan_mode=SCAN_MODE_INTRADAY):
    ...
    for c in candidates:
        ...
        # ── SIGNAL 3: MACD (weight 1.5) ──────────────────────────────────────
        # Must be above signal line. Above zero = much stronger.
        ...
        # ── SIGNAL 4: RSI (weight 1.5) ────────────────────────────────────────
        # Best zone for 2-3 day swing: 50-68 (momentum, room to grow)
        rsi = ind["rsi"]
        if 52 <= rsi <= 65:
            bd["rsi"] = S["rsi"]                   # sweet spot
        ...
        # ── SIGNAL 5: VOLUME QUALITY (weight 1.0) ────────────────────────────
        if vol_r >= 3.0:
            bd["volume"] = S["volume"]
        ...
        # ── SIGNAL 6: BOLLINGER BAND POSITION (weight 1.0) ───────────────────
        ...
        # ── SIGNAL 7: CANDLESTICK PATTERN BONUS (weight 0.5) ─────────────────
        # Check today's candle for bullish patterns
        ...
        # Bullish engulfing / strong green candle: body > 60% of range, closed near high
        ...
        # Even if score passes, these conditions mean skip the stock
        hard_reject = False
        if rsi > 78:
            hard_reject = True
            add_log(f"    REJECT {sym.replace('.NS','')} — RSI {rsi:.1f} too overbought")
        if cmp < e20 * 0.97:  # price more than 3% below EMA20
            hard_reject = True
...
def build_and_send_email(stocks, market, scan_mode=SCAN_MODE_INTRADAY):
    ...
    # Build macro intel banner for email
    ...
    <div style="background:{mbg};border:1px solid {mborder};border-radius:10px;padding:14px 18px;margin-bottom:16px">
      ...
      {macro_signals_html}
      {"" if not macro.get("bear_keywords") else f'<div style="margin-top:6px;font-size:11px;color:#FF9900">Risk flags: {", ".join(macro["bear_keywords"][:4])}</div>'}
      {"" if not macro.get("bull_keywords") else f'<div style="margin-top:3px;font-size:11px;color:#00C853">Positive: {", ".join(macro["bull_keywords"][:3])}</div>'}
    </div>""" if macro else "" 
    ...
    for s in stocks:
        ...
        cards += (
            ...
            '<div>'
            f'<div style="font-size:24px;font-weight:800;color:#FFFFFF;letter-spacing:0.5px">{sym}</div>'
            f'<div style="font-size:12px;color:#546E7A;margin-top:2px">{s["sector"]} · NSE · 2-3 day swing · +{s["chg"]:.2f}% today</div>'
            '</div>'
            '<div style="text-align:right">'
            '<div style="font-size:11px;color:#546E7A;margin-bottom:2px">SIGNAL STRENGTH</div>'
            f'<div style="font-size:26px;font-weight:800;color:{bar_color};line-height:1">'
            f'{s["score"]:.1f}<span style="font-size:12px;color:#546E7A">/{MAX_SCORE:.0f}</span></div>'
            '<div style="background:#1E3248;border-radius:3px;height:4px;margin-top:4px;width:80px;margin-left:auto">'
            f'<div style="background:{bar_color};width:{score_pct}%;height:4px;border-radius:3px"></div>'
            '</div></div></div>'
            ...
            '<td style="padding:14px 20px;color:#78909C;font-size:12px;font-weight:600;'
            'text-transform:uppercase;letter-spacing:0.5px;width:45%">Current Price</td>'
            ...
            f'<td style="padding:11px 20px;text-align:right;color:#64B5F6;font-weight:700;font-size:14px">Rs.{tl["entry_low"]:,.2f} — Rs.{tl["entry_high"]:,.2f}</td>'
            '</tr>'
            '<tr style="background:#071A14;border-top:1px solid #1E3248">'
            f'<td style="padding:11px 20px;color:#78909C;font-size:12px">Target 1 &nbsp;<span style="color:#546E7A;font-size:11px">(book 50%)</span></td>'
            f'<td style="padding:11px 20px;text-align:right;color:#66BB6A;font-weight:800;font-size:16px">Rs.{tl["target1"]:,.2f} <span style="font-size:11px;color:#43A047">+{T1_PCT}%</span></td>'
            '</tr>'
            '<tr style="background:#071A14;border-top:1px solid #1A2E1E">'
            f'<td style="padding:11px 20px;color:#78909C;font-size:12px">Target 2 &nbsp;<span style="color:#546E7A;font-size:11px">(book rest)</span></td>'
            f'<td style="padding:11px 20px;text-align:right;color:#81C784;font-weight:800;font-size:18px">Rs.{tl["target2"]:,.2f} <span style="font-size:11px;color:#43A047">+{T2_PCT}%</span></td>'
            '</tr>'
            '<tr style="background:#1A0A0A;border-top:1px solid #2E1A1A">'
            '<td style="padding:11px 20px;color:#78909C;font-size:12px">Stop Loss &nbsp;'
            '<span style="color:#B71C1C;font-size:11px">(exit immediately if hit)</span></td>'
            f'<td style="padding:11px 20px;text-align:right;color:#EF5350;font-weight:800;font-size:15px">Rs.{tl["stop_loss"]:,.2f} <span style="font-size:11px;color:#E53935">-{SL_PCT}%</span></td>'
            '</tr>'
            '</table></div>'

            # Key stats
            '<div style="background:#0A1929;padding:10px 20px;border-top:1px solid #1E3248;display:flex;gap:20px;flex-wrap:wrap">'
            ...
            f'<span style="font-size:12px;color:#546E7A">Volume <strong style="color:#ECEFF1">{s["vol_ratio"]:.1f}x</strong></span>'
            f'<span style="font-size:12px;color:#546E7A">EMA20 <strong style="color:#ECEFF1">Rs.{ind["ema20"]:,.0f}</strong></span>'
            f'<span style="font-size:12px;color:#546E7A">EMA50 <strong style="color:#ECEFF1">Rs.{ind["ema50"]:,.0f}</strong></span>'
            '</div>'
    ...
    html = (
        ...
        f'<div style="font-size:10px;color:{mode_color};letter-spacing:2px;font-weight:700;margin-bottom:6px">STOCKSENSE AI · AUTO SCANNER</div>'
        f'<div style="font-size:28px;font-weight:800;color:#FFFFFF;line-height:1.2">{hdr_title}</div>'
        f'<div style="font-size:12px;color:#546E7A;margin-top:8px">{now} &nbsp;·&nbsp; {len(stocks)} stock{"s" if len(stocks)!=1 else ""} found</div>'
        '<div style="margin-top:10px;font-size:11px;color:#37474F">'
        'Macro · Nifty · Sector · Earnings · Volume · RSI · MACD · Candle'
        '</div></td></tr>'
        ...
        'Risk max 2% of capital per trade. Place stop loss immediately after buying.'
        '</div></td></tr>'
...
def run_pipeline(force=False, scan_mode=SCAN_MODE_INTRADAY):
    ...
    try:
        # ── Filter 0: Global Macro Intelligence ───────────────────────────────
        ...
        if macro["skip_scan"]:
            ...
            send_macro_caution_email(macro)  # warn but still scan with high bar

        # ── Filter 1-4: Market health + stock scan ────────────────────────────
        market     = run_market_health_checks()
        # Merge macro sector signals into market
        ...
        # Apply macro-adjusted min score
        market["min_score_used"] = max(market["min_score_used"], macro["min_score_adj"])
        add_log(f"Effective min score: {market['min_score_used']:.1f} (macro adj: {macro['min_score_adj']:.1f})")
        ...
        # ── ALWAYS register positions for exit monitoring (regardless of email status) ──
        ...
        if sent:
            ...
            state["last_result"] = f"Email failed - positions still tracked ({len(finalists)} stocks)"
    except Exception as e:
...
<main>
  ...
  <div style="margin-bottom:18px">
    {% for t in ["09:45","10:25","11:30","13:30","14:45","17:30","21:15"] %}
...
def start_scheduler():
    ...
    # 10:25 — Early session: breakouts confirmed after first hour
    ...
    # 14:45 — Power hour: strong close setups, last chance for entry
    ...
    # 17:30 — After 3:30 PM close + 2hrs: all data settled, scan for
    #          tomorrow's swing setups based on today's close patterns
    scheduler.add_job(run_postmarket, CronTrigger(hour=17, minute=30, day_of_week=WD, timezone=IST), id="scan_1730")
    ...
    scheduler.start()
    add_log(
        "Scheduler ready | INTRADAY: 09:45 10:25 11:30 13:30 14:45 IST | "
        "POST-MKT: 17:30 21:15 IST | EXIT MONITOR: every 10 min | self-ping every 10 min | Mon-Fri"
    )
    return scheduler
