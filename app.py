...
<table width="600" style="max-width:600px;width:100%">
  ...
  <tr><td style="background:#0B1826;border-radius:0 0 14px 14px;padding:18px 28px;border-top:1px solid #1A2B3C">
    ...
    if not resend_key:
        add_log("❌ RESEND_API_KEY not set in Render environment variables")
        return False
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            add_log(f"✅ Email sent via Resend → {recips}")
            return True
        else:
            add_log(f"❌ Resend error {resp.status_code}: {resp.text[:300]}")
            return False
    except Exception as e:
        add_log(f"❌ Resend failed: {e}")
        return False
